...
<%         
            ...
            try{ 
                ...
                { 
	                ...
	                /*
	                cookies[i].getName():   ？？？
	                cookies[i].getValue():得到cookies[i]里面存放的数值
	                cookies[i].getValue().split("-")[0]   这是表示什么意思？
	                */
                }
				//若cookie为空,则从表单中提取
			...
			{	
				try{
						...
						String sql="SELECT * FROM User WHERE User_Account=? and User_Password=password(?)";

						ResultSet rs=null;	
						PreparedStatement ps = conn.prepareStatement(sql);
						//PreparedStatement的setString(n,参数)：将第n个？用传进的参数代替
						...
						if(code_error==null)//如果验证码错误，则提示“验证码错误”，不提示“用户名或者密码错误”
						{
		   					if(login_flag)
		   					{
	                            ...
	                            response.sendRedirect("./Friend_Index/Friend_index.jsp");
		                    }
		                    else{
...
<html>
	...
	<body>	
		<div data-role="page" style="background:url('Friend_Index/img/picture.jpg') 50% 0 no-repeat; background-size:cover;" data-theme="a">	
			<link rel="stylesheet" type="text/css" href="./Friend_Index/css/login.css">
			<div class="container" data-role="content" style="margin-top:80px;">			    
				<form method="post" style="margin-top:50px" name="Login_Account">
					<div>				
						<input id="user_id" name="user_name" type="text" maxlength="20" placeholder="用户账号">
					</div>
					...
					<div style="height:80px;width:300px;margin-top:50px;margin-left:auto;margin-right:auto">
						<div style="width:150px;float:left;text-align:center">
							<img src="Friend_validate.jsp" name="validatecode" style="cursor:pointer;" onclick="refresh()" ><br>
					...
					<div style="margin-top:20px">	
						<a href="Friend_register.jsp"  style="float:right">注册账号</a>
					</div>
			...
			function refresh(){
				Login_Account.validatecode.src="Friend_validate.jsp?"+Math.random();
			}
